...
func GlueNameFilter(names []string) FilterPredicate[yomod.ResourceRecord] {
	nameMap := make(map[model.DomainName]any)
	for _, name := range names {
		nameMap[model.MustNewDomainName(name)] = nil
	}

	return func(rr yomod.ResourceRecord) bool {
		_, ok := nameMap[model.MustNewDomainName(rr.Name)]
		return ok
	}
}

func GlueTypeFilter(types []uint) FilterPredicate[yomod.ResourceRecord] {
	typeMap := make(map[uint16]any)
	for _, t := range types {
		typeMap[uint16(t)] = nil
	}
	...
	return func(rr yomod.ResourceRecord) bool {
		_, ok := typeMap[rr.Type]
		return ok
	}
}

func GlueClassFilter(classes []uint) FilterPredicate[yomod.ResourceRecord] {
	classMap := make(map[uint16]any)
	for _, t := range classes {
		classMap[uint16(t)] = nil
	}
